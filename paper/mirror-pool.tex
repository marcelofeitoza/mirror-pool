...
with two live soaks passing $18/18$ behavioral and $27/27$ confidential on-chain
assertions against exactly the bytecode deployed there, backed by $317$ host tests
and $121$ on-chain program tests.
\end{abstract}
...
verifying key. In the exported key only \code{vk\_delta\_2} differs from the initial
key, as phase-2 theory predicts, and a ceremony-key proof is correspondingly rejected
by the initial key's verifying key. \code{snarkjs groth16 verify} accepts the same proof under
the exported \code{verification\_key.json}.

\paragraph{What has been done, and what has not.} The ceremony has been \emph{run}
for \textbf{all three} circuits, and every committed and deployed verifying key is
a ceremony output. Each was opened over the same public 55-contribution
powers-of-tau, contributed to once with OS randomness plus user entropy, and closed
with a Solana mainnet-beta blockhash. No dev-setup key is deployed. All three
transcripts are published in the repository under \code{docs/ceremony-run/} and
re-verify from those files alone with the beacon values supplied; the devnet program
was upgraded in place to bytecode pinning their SHA-256 digests.

\begin{center}
\small
\begin{tabular}{@{}llcl@{}}
\toprule
\textbf{circuit} & \textbf{beacon slot} & \textbf{pre-committed?} & \textbf{final transcript hash} \\
\midrule
membership  & 435825712 & no  & \code{884c8860\ldots 7a9cdd05} \\
transaction & 435846661 & \textbf{yes} & \code{6d044934\ldots 81bbaa80} \\
association & 435846661 & \textbf{yes} & \code{5ef80404\ldots f0808e1d} \\
\bottomrule
\end{tabular}
\end{center}

The \emph{pre-committed} column is the part that matters, and the two answers are
not averaged together. A beacon only removes the last contributor's ability to
grind the final key if its source was fixed before the value could be known;
otherwise the operator can wait and close on whichever of several candidate blocks
suits them. For the JoinSplit and association ceremonies the source was fixed
first: \code{docs/ceremony-run/BEACON-PRECOMMITMENT.md} was written and pushed at
slot \code{435842661}, naming slot \code{435846661} and the exact source string,
roughly 25 minutes before that block existed. The slot was produced
(\code{67Y5hxUd\ldots zWLgH}), so the commitment was honoured as written rather than
falling back to a later slot. The membership ceremony ran earlier and chose its
slot \emph{after} its contribution, so for that key a verifier can confirm the last
step is the announced beacon but cannot rule out that a favourable block was
selected. Run without a beacon value at all, \code{ceremony verify} reports
\code{beacon pre-commitment: NOT supplied} rather than implying the check ran.

What none of the three buys is a distributed trust assumption: each has
\textbf{one} independent contributor, so 1-of-N honest reduces to that single party
having destroyed their scalar (Section~\ref{sec:limits}).
...
\paragraph{Which bytecode the tables belong to.} The program at that address was
\emph{upgraded in place} (signature \code{27Hg4jV8\ldots VLbF}, slot
\code{479622920}) to the current build, the dumped on-chain bytes are byte-for-byte
the local \code{mirror\_pool.so} (SHA-256 \code{5b8cfdc0\ldots 4c3fa7d93}), and both
soaks were then \emph{re-run against it}. Every signature and CU figure below
therefore describes the program a reviewer inspects on the Explorer today. That
build carries three things the pre-registry one did not: verifying keys live in
write-once, digest-pinned accounts instead of compile-time constants; the ZK escrow
is domain-separated from the crowd leaf and bound to a fixed denomination; and all
three verifying keys are the ceremony keys of Section~\ref{sec:ceremony}. Assertion
counts moved with it, from $17$ to $18$ and from $25$ to $27$: the added rows are
the verifying-key publication and, on the confidential side, the address lookup
table that keeps a \code{Transact} inside one $1232$-byte packet now that the
instruction carries the registry account.
...
\subsection{Behavioral soak: crowd $+$ ZK settlement ($18/18$)}
...
All $18/18$ assertions passed, the eighteenth being the publication of the
membership verifying key into its write-once registry.
...
\midrule
\code{init\_pool}          & 20{,}461 & \code{underfloor\_commit}   & 43{,}467 to 44{,}824 \\
\code{crowd\_commit} (x4)  & 38{,}959 to 40{,}331 & \code{crowd\_settle}        & 18{,}733 \\
\code{zk\_deposit\_commit} & 40{,}957 & \code{zk\_settle} (Groth16) & 106{,}101 \\
\bottomrule
\end{tabular}
...
runs in about 106K CU, comfortably inside a transaction's budget alongside the
settlement work. Publishing the membership verifying key (\code{init\_vk}) costs
$5{,}472$ CU, once per deployment.}
\end{table}

\subsection{Confidential-value soak: shield / transfer / unshield ($27/27$)}
...
\text{net deposit} - \text{net withdrawal}$). All $27/27$ assertions passed; the
two beyond the previous $25$ are the verifying-key publication and the lookup
table that keeps a \code{Transact} inside one packet.
...
\textbf{flow} & \textbf{CU} & \textbf{flow} & \textbf{CU} \\
\midrule
\code{init\_value\_pool\_main}  & 24{,}476 & \code{transfer} (JoinSplit) & 193{,}946 \\
\code{init\_value\_pool\_denom} & 23{,}015 & \code{unshield} (JoinSplit) & 195{,}713 \\
\code{shield} (JoinSplit)       & 198{,}587 & \code{denom\_shield\_exact}  & 205{,}407 \\
\bottomrule
\end{tabular}
...
about 194K to 205K CU. Publishing the JoinSplit verifying key (\code{init\_vk})
costs $4{,}108$ CU, once per deployment.}
\end{table}

\paragraph{Test suite.} Beyond the two live soaks, the repository ships
\textbf{317 host tests} (\code{cargo test --workspace}, covering the Poseidon
...
trusted-setup ceremony including the three deployed transcripts, and the
effective-$k$ and funding-model functions; $13$ further tests are
environment-gated and skipped by default) and \textbf{121 on-chain program tests} (mollusk integration tests plus in-crate unit tests).
...
\begin{itemize}[leftmargin=1.4em,itemsep=0.15em]
  \item \textbf{All three deployed trusted setups are real ceremonies, each with
        ONE independent contributor.} Every deployed verifying key is the output of
        a phase-2 ceremony over the same public 55-contribution powers-of-tau,
        closed by a Solana mainnet-beta blockhash beacon
        (Section~\ref{sec:ceremony}); no dev-setup key is deployed, so no toxic
        waste is public by construction any more. What remains is that each
        ceremony had \textbf{one} independent contributor, which is a single point
        of trust rather than a distributed one: sound only if that one party
        destroyed their scalar. Anyone unwilling to extend that trust should treat
        the proof system as unsound for their purposes. The JoinSplit and
        association ceremonies additionally closed on a beacon slot pre-committed
        in public before its value existed; the membership one, deployed earlier,
        did not, so for that key slot-shopping cannot be ruled out. Re-running all
        three with external, mutually independent contributors is the outstanding
        work. This is a soundness caveat, disclosed here rather than buried.
  ...
  \item \textbf{The funding path is not wired and not soaked.} The unshield it is
        built on is soak-proven on devnet ($27/27$), but the layer above it is two
...
which needs a per-circuit trusted setup (Section~\ref{sec:ceremony}; all three
deployed keys come from real, but single-contributor, ceremonies). Approaches built on
...
\paragraph{Public devnet soaks (the $18/18$ and $27/27$ assertion tables and the
CU numbers).}
\begin{quote}\footnotesize\ttfamily
