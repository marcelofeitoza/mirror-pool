...
zero-knowledge opt-in path proves membership on-chain via Groth16 and settles
with no participant signature, giving cryptographic who-initiated unlinkability
within the set its own public inputs leave standing (Section~\ref{sec:limits}). The optional confidential-value layer adds a
...
of about $7.5$ (worst case $1$) under funding-provenance partitioning. We close
that channel with a mechanism rather than an assumption - commit wallets are
funded by unshielding from the value pool in denominated, batched funding rounds
- and then measure what the mechanism leaves: $28.80$ of $32$ ($90.0\%$), with the
missing $10\%$ published as a residual amount/timing leak rather than rounded away. Everything is deployed and browser-verifiable on
...
assertions, backed by $205$ host tests and $42$ on-chain program tests.
\end{abstract}
...
\begin{itemize}[leftmargin=1.4em,itemsep=0.15em]
  ...
  \item \textbf{A funding-provenance mechanism, and its measured residual}: the
        ...
        anonymity from that mechanism instead of assuming it, and publishes the
        leak that survives.
...
alone: \code{SettleZk} publishes the epoch and the amount, so the covering set is
the window's ZK deposits of the same amount, and the program enforces no floor on
it, no denomination, and no recipient freshness. Section~\ref{sec:limits} states
each of those and why none can be a settle-time check on a path whose escrow has
no refund.
...
from the confidential-value pool (\code{mirror-cli fund-commit}), and the
coordinator releases those withdrawals in batched \emph{funding rounds}
(\code{mirror\_coordinator::funding}). The on-chain sender is the pool vault and
the relay is the only signature, so the main-wallet-to-commit-wallet edge is never
written. What survives is \emph{not} nothing: \code{publicAmount} is public on both
boundary crossings, so an observer sees the deposits and the withdrawals and is
left with a \textbf{matching problem}. Our effective-$k$ model derives the
MirrorPool provenance classes from exactly that matching, which is what makes the
result a measurement rather than a restatement of the design intent.
...
MirrorPool: denominated $+$ rounds & 32 & \textbf{28.80} & 21.00 & \textbf{90.0\%} \\
\addlinespace
...
$\mathbf{28.80}$ of $32$. The missing $\mathbf{10.0\%}$ is the residual
amount/timing channel of the public boundary crossings, and the worst-placed
committer sits at $3.00$, not $32$. MirrorPool's all-channel rows equal its
provenance-only rows because settlement exposes no per-actor timing, amount, or
fee shape.}
\end{table}
...
$24.25 / 27.43 / 28.80 / 30.01 / 30.88$, i.e. diminishing returns past the default
of $2$. Adoption bounds everything: at $90\%/75\%/50\%/25\%$ adoption the same
configuration measures $24.65 / 20.07 / 13.56 / 9.33$, and as soon as one
...
$k{=}64$: $19.57 \to 17.33$) and small under the shipped default
...
bound to the running transcript hash, the contribution index, and the contributor
identifier; without extractable knowledge of the ratio, a contributor could publish a
point derived from an earlier step and cancel an honest contributor's randomness.
Entries are chained with SHA-256 over a canonical, length-prefixed serialization.
...
knowledge, a reordered chain, and a truncated chain; each rejection has a test,
...
\textbf{205 host tests} (\code{cargo test --workspace}, covering the Poseidon
...
trusted-setup ceremony, and the effective-$k$ and funding-model functions) and \textbf{42 on-chain program tests} (mollusk integration tests
...
\begin{itemize}[leftmargin=1.4em,itemsep=0.15em]
  ...
  \item \textbf{Effective-$k$ is a model.} The numbers come from deterministic
        ...
        roadmap item.
  \item \textbf{The funding path is unit-tested, not soaked.} The unshield it is
        built on is soak-proven on devnet ($25/25$), but the \code{fund-commit}
        plus funding-round orchestration on top of it is covered by host tests
        only; the two live soaks predate it. A funding-round soak is named future
        work, not an implied result.
  \item \textbf{Funding provenance is reduced, not closed.} The funding mechanism
        turns a public edge into a matching problem, and the measurement says how
        much of that matching survives: $90.0\%$ of nominal at $k{=}32$ under
        denominated batched rounds, so a $10.0\%$ residual, dropping to $23.9\%$ of
        nominal if participants pass value straight through a free-amount pool and
        to $42.4\%$ at $50\%$ adoption. The mechanism also does not hide that a
  ...
  \item \textbf{The ZK escrow is a shared pot (v1 soundness).} The leaf is opaque,
        so nothing on-chain ties a settled amount to what its owner escrowed, and
        because both paths append the same leaf shape to one accumulator, a
        fee-only crowd \code{Commit} leaf satisfies the membership circuit too (a
        test demonstrates exactly this). A v1 pool must therefore not hold value it
        cannot afford to lose. Closing it needs a fixed denomination plus
        domain-separated leaves, a layout and circuit change; a partial fix would
...
\paragraph{Tests (177 host, of which 7 are environment-gated; 42 program).}
\begin{quote}\footnotesize\ttfamily
...
\ \ -{}-r1cs circuits/membership.r1cs -{}-initial-zkey circuits/membership\_0000.zkey\\
...
shielded funding, $28.80$ for denominated batched rounds; the funding-policy and
adversary-strength ablations; the dwell and adoption sweeps; and the
