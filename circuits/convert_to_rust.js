...
function main() {
  const vk = JSON.parse(fs.readFileSync(path.join(ARTIFACTS, "verification_key.json"), "utf8"));
  const fixture = JSON.parse(fs.readFileSync(path.join(ARTIFACTS, "proof_fixture.json"), "utf8"));
  const meta = JSON.parse(fs.readFileSync(path.join(ARTIFACTS, "fixture_meta.json"), "utf8"));
  ...
  s += "// mirror-pool membership verifying key, groth16-solana v0.2.0 layout.\n";
  ...
  s += "    ],\n";
  s += "};\n";
  fs.writeFileSync(path.join(ARTIFACTS, "vk.rs"), s);
  ...
  p += "// Known-good mirror-pool membership proof + public inputs, groth16-solana v0.2.0 layout.\n";
  ...
  p += "];\n";
  fs.writeFileSync(path.join(ARTIFACTS, "proof_fixture.rs"), p);

  console.log("Wrote artifacts/vk.rs and artifacts/proof_fixture.rs");
