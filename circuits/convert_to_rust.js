...
// The SHIELD + UNSHIELD constants the on-chain transact tests submit, written to
// programs/mirror-pool/tests/fixtures/transaction_extra.rs.
//
// The canonical TRANSFER fixture becomes artifacts/transaction_proof_fixture.rs
// above; these two are the other public-amount directions, and they live next to
// the tests that consume them rather than under artifacts/. This mode exists so
// that file is REGENERABLE from the committed fixtures: it used to be produced by
// an uncommitted scratchpad script, which meant a proving-key rotation silently
// left it stale and four mollusk tests failing with no documented way to fix it.
function writeTransactionExtra() {
  const OUT = path.join(
    __dirname,
    "..",
    "programs",
    ...
    "tests",
    "fixtures",
    "transaction_extra.rs"
  );
  const cases = [
    ["SHIELD", "transaction_shield_fixture.json"],
    ["UNSHIELD", "transaction_unshield_fixture.json"],
  ];

  let s = "";
  s += "// @generated by circuits/convert_to_rust.js --transaction-extra - DO NOT EDIT BY HAND.\n";
  s += "// SHIELD (publicAmount=+10, 2 dummy inputs) and UNSHIELD (publicAmount=r-7).\n";
  s += "// Both are the committed circuits/artifacts fixtures, in the groth16-solana\n";
  s += "// layout: PROOF_A already negated, PROOF_B imaginary-part-first.\n";
  for (const [name, file] of cases) {
    const fx = JSON.parse(fs.readFileSync(path.join(ARTIFACTS, file), "utf8"));
    const a = g1Neg(fx.proof.pi_a);
    const b = g2(fx.proof.pi_b);
    const c = g1(fx.proof.pi_c);
    const pub = fx.publicSignals.map((x) => toBE32(x));
    s += `pub const ${name}_PROOF_A: [u8; 64] = [\n` + fmtBytes(a, "    ") + "\n];\n";
    s += `pub const ${name}_PROOF_B: [u8; 128] = [\n` + fmtBytes(b, "    ") + "\n];\n";
    s += `pub const ${name}_PROOF_C: [u8; 64] = [\n` + fmtBytes(c, "    ") + "\n];\n";
    s += `pub const ${name}_PUBLIC_INPUTS: [[u8; 32]; ${pub.length}] = [\n`;
    for (const inp of pub) {
      s += "    [\n" + fmtBytes(inp, "        ") + "\n    ],\n";
    }
    s += "];\n";
  }
  ...
  console.log("  SHIELD + UNSHIELD proof triples and 7 public inputs each");
}

function main() {
  // Optional --transaction-extra: regenerate the on-chain tests' SHIELD/UNSHIELD
  // constants instead of an artifacts/ pair.
  if (process.argv.slice(2).includes("--transaction-extra")) {
